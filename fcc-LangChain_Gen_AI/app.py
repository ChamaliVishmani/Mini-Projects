...
if refined_text_prompt is not None:
    ...
    if generate_image_btn:
        ...
        st.markdown(f"[Download Image]({output[0]})", unsafe_allow_html=True)
        st.download_button(label="Download Image", data=output[0])

img_input = st.file_uploader("Upload an image", type=["jpg", "png"])
...
if variation_Btn:
    ...
    response = model.generate_content(
        ["use keywords "+variation_keywords+" and write a image generation text prompt for the image given with the conetxt"+variation_input, img], stream=True)
    response.resolve()
